...
std::shared_ptr<GaussianProcess> GaussianProcessFactory::LoadGaussianProcessFromJson(const Json &json) {
    ...
    if (gpType == "std") {
        auto meanFunction = MeanFunctionFactory::LoadMeanFunctionFromJson(json);
        auto covarianceFunction = CovarianceFunctionFactory::LoadCovarianceFunctionFromJson(json);
