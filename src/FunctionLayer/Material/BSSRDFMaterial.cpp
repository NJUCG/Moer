...
std::shared_ptr < BxDF > BssrdfMaterialBase::getBxDF(const Intersection & intersect) const {
    return bsdfMaterial->getBxDF(intersect);
}

BssrdfMaterialBase::BssrdfMaterialBase(const Json & json) {
    bsdfMaterial = MaterialFactory::LoadMaterialFromJson(json["bsdf"]);
}

std::shared_ptr < BSSRDF > BssrdfMaterial::getBSSRDF(const Intersection & intersect) const {
    return std::make_shared < TabelBSSRDF >(eta, sigmaA->eval(intersect) * scale , sigmaS->eval(intersect) * scale , g, table, intersect);
}

struct MeasuredSS {
    const char *name;
    RGB3 sigma_prime_s, sigma_a;  // mm^-1
};
...
BssrdfMaterial::BssrdfMaterial(const Json & json) : BssrdfMaterialBase(json),
        eta(getOptional(json, "eta", 1.5)), g(getOptional(json, "g", 0)), table(100, 64) {
    sigmaA = TextureFactory::LoadTexture(json, "sigma_a", Spectrum(1));
    sigmaS = TextureFactory::LoadTexture(json, "sigma_s", Spectrum(1));
    scale = getOptional(json,"scale",1.0);
  //  if(json.contains("material")){
        Spectrum rgbSigmaA,rgbSigmaS;
        GetMediumScatteringProperties("Skin1",&rgbSigmaA,&rgbSigmaS);
        sigmaA = std::make_shared <ConstantTexture<Spectrum>>(rgbSigmaA);
        sigmaS = std::make_shared <ConstantTexture<Spectrum>>(rgbSigmaS);
   // }
    computeBeamDiffusionBSSRDF(g, eta, & table);
    twoSideShading = false;
}

BSSRDFAdapterMaterial::BSSRDFAdapterMaterial(const SeparableBSSRDF * bssrdf) : bssrdf(bssrdf) {
    twoSideShading = false;
}

std::shared_ptr < BxDF > BSSRDFAdapterMaterial::getBxDF(const Intersection & intersect) const {
    return std::make_shared <BSSRDFAdapter>(bssrdf);
}

std::shared_ptr < BSSRDF > KdSubsurfaceMaterial::getBSSRDF(const Intersection & intersect) const {
    Spectrum kt = Spectrum(RGB3(0.5,0.5,0.5));
    Spectrum mfree = mfp->eval(intersect);
    Spectrum sigmaA,sigmaS;
    SubsurfaceFromDiffuse(table,kt,mfree,&sigmaA,&sigmaS);
    return std::make_shared < TabelBSSRDF >(eta,sigmaA,sigmaS,g,table,intersect);
}

KdSubsurfaceMaterial::KdSubsurfaceMaterial(const Json & json) : BssrdfMaterialBase(json),
eta(getOptional(json, "eta", 1.5)), g(getOptional(json, "g", 0)), table(100, 64){
    Kt = TextureFactory::LoadTexture(json, "Kt", Spectrum(1));
    mfp = TextureFactory::LoadTexture(json,"mfp",Spectrum(1));
    computeBeamDiffusionBSSRDF(g, eta, & table);
    twoSideShading = false;
}
