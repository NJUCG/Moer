...
bool HomogeneousMedium::sampleDistanceDeltaTracking(MediumSampleRecord *mRec,
                                                    const Ray &ray,
                                                    const Intersection &its,
                                                    Point2d sample) const {
    auto [x, y] = sample;
...
Spectrum HomogeneousMedium::evalTransmittanceRatioTracking(Point3d from,
                                                           Point3d dest) const {
    double dist = (dest - from).length();
    Spectrum tr;
    for (int i = 0; i < nSpectrumSamples; i++) {
