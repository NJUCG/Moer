...
TEST_CASE("test-integrator-validate")
{
    ...
    auto sphereEmitterShape = std::make_shared<Sphere>(Point3d(-6, 2, -6), 1.0, lambert);
    ...
    scene->addEntity(std::make_shared<Sphere>(Point3d(0.0, 0.0, 50.0), 40.0, lambert));
    scene->addEntity(std::make_shared<Sphere>(Point3d(0.0, 0.0, -50.0), 40.0, lambert));
    scene->addEntity(std::make_shared<Sphere>(Point3d(-50.0, 0.0, 0.0), 40.0, lambert));
    scene->addEntity(std::make_shared<Sphere>(Point3d(50.0, 0.0, 0.0), 40.0, lambert));
    scene->addEntity(std::make_shared<Sphere>(Point3d(0.0, -101.0, 0.0), 100.0, lambert));
    std::cout << "scene created" << std::endl;
    scene->addLight(std::make_shared<PointLight>(100.0, Point3d(8, 2, -6)));
    ...
    PathIntegrator integrator(pinhole, std::make_unique<Film>(Point2i(128, 128), 3), nullptr, std::make_shared<DirectSampler>(), 1024);
