...
///
/// @note NONE is the default, ZIP is slow but compact and BLOSC offers a great
/// balance.
///
/// @warning NanoVDB optionally supports ZIP and BLOSC compression and will
/// throw an exception
...
/// @brief Internal functions for compressed read/write of a NanoVDB GridHandle
/// into a stream
///
...
static fileSize_t write(std::ostream &os, const GridHandle<BufferT> &handle,
                        Codec codec);
...
/// @brief Return a uint64_t with its bytes reversed so we can check for
/// endianness
...
struct Segment {
  ...
  Segment(Codec c = Codec::NONE) : header(c), meta() {}
  template <typename BufferT> void add(const GridHandle<BufferT> &h);
  bool read(std::istream &is);
  void write(std::ostream &os) const;
  uint64_t memUsage() const;
}; // Segment

/// @brief Write a single grid to file (over-writing existing content of the
/// file)
template <typename BufferT>
void writeGrid(const std::string &fileName, const GridHandle<BufferT> &handle,
               Codec codec = Codec::NONE, int verbose = 0);
...
template <typename BufferT>
void writeGrid(std::ostream &os, const GridHandle<BufferT> &handle,
               Codec codec = Codec::NONE);

/// @brief Write multiple grids to file (over-writing existing content of the
/// file)
template <typename BufferT = HostBuffer,
          template <typename...> class VecT = std::vector>
...
template <typename BufferT = HostBuffer,
          template <typename...> class VecT = std::vector>
void writeGrids(std::ostream &os, const VecT<GridHandle<BufferT>> &handles,
                Codec codec = Codec::NONE);
...
template <typename BufferT = HostBuffer>
GridHandle<BufferT> readGrid(const std::string &fileName, uint64_t n = 0,
                             int verbose = 0,
...
template <typename BufferT = HostBuffer>
GridHandle<BufferT> readGrid(std::istream &is, uint64_t n = 0,
                             const BufferT &buffer = BufferT());

/// @brief Read the first grid with a specific name
///
/// @warning If not grid exists with the specified name the resulting GridHandle
/// is empty
template <typename BufferT = HostBuffer>
GridHandle<BufferT> readGrid(const std::string &fileName,
                             const std::string &gridName, int verbose = 0,
                             const BufferT &buffer = BufferT());

/// @brief Read the first grid with a specific name
template <typename BufferT = HostBuffer>
GridHandle<BufferT> readGrid(std::istream &is, const std::string &gridName,
                             const BufferT &buffer = BufferT());
...
/// @brief Reads and returns a vector of meta data for all the grids found in
/// the specified file
...
template <typename BufferT>
fileSize_t Internal::write(std::ostream &os, const GridHandle<BufferT> &handle,
                           Codec codec) {
  ...
  case Codec::ZIP: {
#ifdef NANOVDB_USE_ZIP
    uLongf size = compressBound(
        residual); // Get an upper bound on the size of the compressed data.
    std::unique_ptr<Bytef[]> tmp(new Bytef[size]);
    const int status = compress(
        tmp.get(), &size, reinterpret_cast<const Bytef *>(data), residual);
    if (status != Z_OK)
...
#ifdef NANOVDB_USE_BLOSC
    do {
      fileSize_t chunk = residual < MAX_SIZE ? residual : MAX_SIZE,
                 size = chunk + BLOSC_MAX_OVERHEAD;
      std::unique_ptr<char[]> tmp(new char[size]);
      const int count =
          blosc_compress_ctx(9, 1, sizeof(float), chunk, data, tmp.get(), size,
                             BLOSC_LZ4_COMPNAME, 1 << 18, 1);
      if (count <= 0)
...
template <typename BufferT>
void Internal::read(std::istream &is, GridHandle<BufferT> &handle,
                    Codec codec) {
  ...
  case Codec::ZIP: {
    ...
    int status = uncompress(reinterpret_cast<Bytef *>(data), &numBytes,
                            tmp.get(), static_cast<uLongf>(size));
    if (status != Z_OK)
  ...
  case Codec::BLOSC: {
    ...
    do {
      ...
      const int count = blosc_decompress_ctx(tmp.get(), data, size_t(chunk),
                                             1); // fails with more threads :(
      if (count < 1)
...
template <typename ValueT>
inline GridMetaData::GridMetaData(uint64_t size, Codec c,
  ...
  const uint32_t *ptr =
      reinterpret_cast<const TreeData<3> *>(&grid.tree())->mNodeCount;
  for (int i = 0; i < 3; ++i) {
...
inline bool Segment::read(std::istream &is) {
  ...
  if (!is || header.magic != NANOVDB_MAGIC_NUMBER) {
    ...
    if (header.version.getMajor() < NANOVDB_MAJOR_VERSION_NUMBER) {
      ss << "The file contains an older version of NanoVDB: "
         << std::string(header.version.c_str()) << "!\n\t"
         << "Recommendation: Re-generate this NanoVDB file with this version: "
         << NANOVDB_MAJOR_VERSION_NUMBER << ".X of NanoVDB";
    } else {
      ss << "This tool was compiled against an older version of NanoVDB: "
         << NANOVDB_MAJOR_VERSION_NUMBER << ".X!\n\t"
         << "Recommendation: Re-compile this tool against the newer version: "
         << header.version.getMajor() << ".X of NanoVDB";
    }
    throw std::runtime_error(
...
template <typename BufferT>
void writeGrid(const std::string &fileName, const GridHandle<BufferT> &handle,
               Codec codec, int verbose) {
  std::ofstream os(fileName,
                   std::ios::out | std::ios::binary | std::ios::trunc);
  if (!os.is_open()) {
    throw std::runtime_error("Unable to open file named \"" + fileName +
                             "\" for output");
  }
  writeGrid<BufferT>(os, handle, codec);
  if (verbose) {
    std::cout << "Wrote nanovdb::Grid to file named \"" << fileName << "\""
              << std::endl;
  }
}
...
void writeGrids(const std::string &fileName,
                const VecT<GridHandle<BufferT>> &handles, Codec codec,
                int verbose) {
  std::ofstream os(fileName,
                   std::ios::out | std::ios::binary | std::ios::trunc);
  if (!os.is_open()) {
...
template <typename BufferT>
GridHandle<BufferT> readGrid(const std::string &fileName, uint64_t n,
                             int verbose, const BufferT &buffer) {
  ...
  if (verbose) {
    std::cout << "Read NanoGrid # " << n << " from the file named \""
              << fileName << "\"" << std::endl;
  }
  return handle; // is converted to r-value and return value is move
                 // constructed.
}

template <typename BufferT>
GridHandle<BufferT> readGrid(std::istream &is, uint64_t n,
  ...
  while (s.read(is)) {
    ...
    for (auto &m : s.meta) {
      if (counter == n) {
        ...
        is.seekg(seek,
                 std::ios_base::cur); // skip forward from the current position
...
template <typename BufferT>
GridHandle<BufferT> readGrid(const std::string &fileName,
                             const std::string &gridName, int verbose,
                             const BufferT &buffer) {
  std::ifstream is(fileName, std::ios::in | std::ios::binary);
  if (!is.is_open()) {
    throw std::runtime_error("Unable to open file named \"" + fileName +
  ...
  if (verbose) {
    if (handle) {
      std::cout << "Read NanoGrid named \"" << gridName
                << "\" from the file named \"" << fileName << "\"" << std::endl;
    } else {
...
GridHandle<BufferT> readGrid(std::istream &is, const std::string &gridName,
  ...
  while (s.read(is)) {
    ...
    for (auto &m : s.meta) {
      if (m.nameKey == key &&
          m.gridName == gridName) { // check for hash key collision
        GridHandle<BufferT> handle(BufferT::create(m.gridSize, &buffer));
...
template <typename BufferT, template <typename...> class VecT>
VecT<GridHandle<BufferT>> readGrids(const std::string &fileName, int verbose,
                                    const BufferT &buffer) {
  std::ifstream is(fileName, std::ios::in | std::ios::binary);
  if (!is.is_open()) {
    throw std::runtime_error("Unable to open file named \"" + fileName +
  ...
  if (verbose) {
    std::cout << "Read " << handles.size()
              << " NanoGrid(s) from the file named \"" << fileName << "\""
              << std::endl;
  }
  return handles; // is converted to r-value and return value is move
                  // constructed.
}
...
inline std::vector<GridMetaData> readGridMetaData(const std::string &fileName) {
  ...
  return readGridMetaData(
      is); // is converted to r-value and return value is move constructed.
}
